# pdf only
slide_buttons <- function(slide_id, output_type = "pdf") {
  out <- if(output_type == "pdf"){
    # pdf button link
    glue::glue('<a class="btn btn-danger" target="_blank" href="{slide_id}.pdf" role="button"><i class="fa-solid fa-file-pdf"></i> Download PDF of all slides</a></p>')
  } else if(output_type == "html"){
    # html button link
    glue::glue('<p class="buttons"><a class="btn btn-danger" target="_blank" href="{slide_id}.html"><i class="fa-solid fa-arrow-up-right-from-square"></i> View all slides in new window</a>')
  }
  else {
    # both html + pdf button links
